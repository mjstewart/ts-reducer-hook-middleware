...
type TestAction = { type: 'log'; log: string };
...
test('middlewares called from left to right when the next dispatch function is called last', () => {
  ...
  expect(testInstance.findAllByType('li')[2].children[0]).toBe(
    'middlewareThree'
  );
...
test('middlewares called from right to left when the next dispatch function is called first', () => {
  ...
  expect(testInstance.findAllByType('li')[1].children[0]).toBe(
    'middlewareThree'
  );
...
test('middlewares can read initial store', () => {
  ...
  const TestCounter: React.SFC = () => {
    const {
      store: initialStore,
      dispatch: enhancedDispatch
    } = useReducerWithMiddleware<typeof testReducer>(testReducer, initialState)(
      [
        api => next => action => {
  ...
  // dispatch effects
  expect(testInstance.findAllByType('li')[1].children[0]).toBe(
    'middlewareOne, initialState'
  );
  expect(testInstance.findAllByType('li')[2].children[0]).toBe(
    'middlewareTwo, initialState'
  );
  expect(testInstance.findAllByType('li')[3].children[0]).toBe(
    'middlewareThree, initialState'
  );

  // last call to the next dispatcher in middlewareThree ends up running the original action supplied to the composed dispatcher.
  expect(testInstance.findAllByType('li')[4].children[0]).toBe(
    'first, initialState'
  );
});
